...
# If fluent-bit exposes a service or metrics endpoint:
kubectl port-forward svc/fluent-bit -n logging 2020:2020 --address=0.0.0.0 &
# Kibana UI:
kubectl port-forward svc/kibana-kibana -n logging 5601:5601 --address=0.0.0.0 &
# Jaeger UI:
kubectl port-forward svc/jaeger-query -n tracing 16686:16686 --address=0.0.0.0 &
```

---

## 12. Install Logging and Tracing Stacks
...
helm install elasticsearch elastic/elasticsearch --namespace logging \
  --set replicas=1 \
  --set volumeClaimTemplate.spec.resources.requests.storage=5Gi \
...
helm install jaeger jaegertracing/jaeger --namespace tracing \
  ...
  --set provisionDataStore.memory=true \
  --set collector.enabled=true \
  --set query.enabled=true \
  --set ui.enabled=true \
  --set query.service.type=NodePort \
  --set query.service.nodePort=32090
